...
export function startHookServer({ port, secret, onRequest, onDrop, log = () => {}, retries = 3, retryMs = 500 }) {
  ...
  const server = createServer((req, res) => {
    ...
    if (!sameSecret(req.url ?? "", wantPath)) { stats.badPath++; return deny(404); }
      ...
      server.listen(port, "127.0.0.1", () => {
        ...
        resolve({
          ...
          close: () => new Promise((done) => {
            ...
            // A held request keeps its socket non-idle, so bound the wait.
            setTimeout(() => { server.closeAllConnections?.(); finish(); }, 250);
          }),
        });
      });
    };
